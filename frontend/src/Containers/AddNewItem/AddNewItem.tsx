...
const AddNewItem = () => {
  ...
  const [newItem, setNewItem] = useState<IItemForm>({
    title: '',
    description: '',
    image: null,
    price: 0,
  ...
  const onFormSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (
      ...
      newItem.title.trim().length !== 0 && newItem.price >= 0 &&
        ...
        <form onSubmit={onFormSubmit} className="w-50 mx-auto">
          ...
          <div className="mb-3">
            <label htmlFor="category">
              <span>Category:</span>
              <select className="form-control" name="category" onChange={changeForm} value={newItem.category}>
